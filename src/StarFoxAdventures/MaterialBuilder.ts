...
export class SFAMaterialBuilder<RenderContext = undefined> {
    ...
    public genTexMap(texture: TexFunc<RenderContext>): TexMap {
        ...
        this.texMaps.push(texture); // Storing the TexFunc here
        return id as TexMap;
    }
    ...
    public setOnMaterialParams(params: MaterialParams, ctx: RenderContext) {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            // CRITICAL CHANGE: Always reset the texture mapping *before* applying the new function.
            // This ensures any lingering GPU texture bindings are cleared.
