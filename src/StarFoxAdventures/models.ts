...
public getModel(num: number): Model {
    if (this.models[num] === undefined) {
        ...
        const rawData = this.bin.subarray(modelOffs + 0x24);
        
        // Inspect first 4 bytes for magic
        const dv = rawData.createDataView();
        const magicBytes = [
            dv.getUint8(0),
            dv.getUint8(1),
            dv.getUint8(2),
...
    public static async create(gameInfo: GameInfo, texFetcher: Promise<TextureFetcher>, materialFactory: MaterialFactory, animController: SFAAnimationController, modelVersion: ModelVersion = ModelVersion.Demo): Promise<ModelFetcher> {
