...
class MyShapeHelper {
    ...
    constructor(device: GfxDevice, cache: GfxRenderCache, public loadedVertexLayout: LoadedVertexLayout, public loadedVertexData: LoadedVertexData, dynamicVertices: boolean, dynamicIndices: boolean) {
       // console.log(`[DEBUG_SHAPE_GPU] MyShapeHelper constructor: Initializing buffers. Vertices: ${loadedVertexData.totalVertexCount}, Indices: ${loadedVertexData.totalIndexCount}`);
        for (let i = 0; i < loadedVertexData.vertexBuffers.length; i++) {
         ...
         //   console.log(`[DEBUG_SHAPE_GPU] MyShapeHelper: Created vertex buffer ${i} (size: ${loadedVertexData.vertexBuffers[i].byteLength}).`);
      ...
      //  console.log(`[DEBUG_SHAPE_GPU] MyShapeHelper: Created input layout.`);
       ...
       // console.log(`[DEBUG_SHAPE_GPU] MyShapeHelper: Created index buffer (size: ${loadedVertexData.indexData.byteLength}).`);
    ...
    public uploadData(device: GfxDevice, uploadVertices: boolean, uploadIndices: boolean) {
        if (uploadVertices) {
            for (let i = 0; i < this.loadedVertexData.vertexBuffers.length; i++) {
              ...
              //  console.log(`[DEBUG_SHAPE_GPU] MyShapeHelper: Uploaded data to vertex buffer ${i}.`);
            }
        }
